...
import br.com.vitrini.controller.ActivityController;
import br.com.vitrini.entities.Vitrini;
import br.com.vitrini.utils.BitmapHelper;
import br.com.vitrini.utils.StringUtils;

public class VitriniListAdapter extends ArrayAdapter<Vitrini> implements Filterable{
	private final List<Vitrini> vitriniItemListData;
	private List<Vitrini> vitriniItemList;
	private LayoutInflater mInflater;
	private ActivityController controller;

	public VitriniListAdapter(Context c, List<Vitrini> objects, Resources resources, ActivityController controller)
	{
		super(c, 0);
...
//		VitriniListAdapter.this.notifyDataSetChanged();
	...
	private View initView( int position, View itemView)
	{
		...
		if (itemView == null)
		{
			itemView = mInflater.inflate(R.layout.vitrini_list_item,
					null);
			...
			vitriniItemView.iconNotes = (ImageView) itemView.findViewById( R.id.notes_icon );
		...
		if(currentVitriniItem.getAnnotation() != null && !currentVitriniItem.getAnnotation().isEmpty()) {
			vitriniItemView.iconNotes.setBackgroundResource(R.drawable.icon_notes_selected);
		} else {
			vitriniItemView.iconNotes.setBackgroundResource(R.drawable.icon_notes);			
		}
	            ...
	            protected FilterResults performFiltering(CharSequence constraint) {
	                ...
	                if (constraint == null || constraint.length() == 0) {
	                    ...
	                    for (Vitrini p : vitriniItemListData) {
	                    	...
	                    	final String handledLeftString = StringUtils.normalize(p.getName()); 
	                    	final String handledRightString = StringUtils.normalize(constraint.toString());
	                    	
							if (handledLeftString
	                                .contains(handledRightString)) {
