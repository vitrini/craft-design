...
import br.com.vitrini.view.adapters.VitriniListAdapter;
...
public class VitriniListActivity extends AbstractActivity 
{
	...
	private List<String> checkedSegments;
	...
	public void initView() {
      ...
      // Verifica quais vitrinis estão sem imagens
    	//		int withoutImages = testImages();
    	//		Log.e("IMAGES", "Number of Vitrinis Without Images : " + withoutImages );
...
//		});
	...
	/**
	 * Testa quantas imagens foram carregadas em cada vitrini
	 * @return Número de vitrinis sem nenhuma imagem
	 */
	private int testImages()
	{
		Log.e("IMAGES", "Vitrinis sem imagens:" );

    	List<Vitrini> vitrinis =  getVitrinis();

		int vitrinisWithoutImages = 0;
		for(int i=0; i<vitrinis.size(); i++)
		{
			final String vitriniName = StringUtils.normalize(vitrinis.get(i).getName());
			ImagePagerAdapterVitrinis imageAdapter = new ImagePagerAdapterVitrinis( getApplicationContext(), vitriniName );
//			Log.e("IMAGES", vitriniName + ": " + imageAdapter.getCount() );

			if (imageAdapter.getCount() == 0)
			{
				Log.e("IMAGES", vitriniName);
				vitrinisWithoutImages++;
			}
		}

		return vitrinisWithoutImages;		
	}
}
